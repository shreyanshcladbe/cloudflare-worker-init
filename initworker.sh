...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Cloudflare Worker Project Setup${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""

# Ask for project name
echo -e "${YELLOW}Enter the name of your project:${NC}"
read -r PROJECT_NAME

if [ -z "$PROJECT_NAME" ]; then
    echo -e "${YELLOW}No project name provided. Exiting.${NC}"
    exit 1
fi
...
echo -e "${BLUE}Running create-cloudflare...${NC}"
echo -e "${YELLOW}This may take a few minutes while dependencies are installed...${NC}"
echo ""
...
npx create-cloudflare@latest --platform=workers "$PROJECT_NAME" \
    ...
    --git=true \
    --deploy=false || {
    echo -e "${YELLOW}Error during create-cloudflare. Please check logs above and try manually if needed.${NC}"
    exit 1
}
...
if [ ! -d "$PROJECT_NAME" ]; then
    echo -e "${YELLOW}Error: Project directory was not created. Exiting.${NC}"
    exit 1
fi

cd "$PROJECT_NAME" || {
    echo -e "${YELLOW}Error: Could not navigate to project directory.${NC}"
    exit 1
}

# Add comment to wrangler.jsonc
echo -e "${BLUE}Configuring wrangler.jsonc...${NC}"
TEMP_FILE=$(mktemp)
